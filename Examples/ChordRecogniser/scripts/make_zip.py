...
def  main():
  ...
  installer = "\installer\ChordRecogniser Installer.exe"
   
  if demo:
    installer = "\installer\ChordRecogniser Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ChordRecogniser manual.pdf" 
  ]
  ...
  ZIPNAME = "ChordRecogniser-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ChordRecogniser-v" + FULLVERSIONSTR + "-win-demo.zip"
