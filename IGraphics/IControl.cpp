...
void IChordTextControl::OnMidi(const IMidiMsg& msg)
{
    switch (msg.StatusMsg())
    {
        case IMidiMsg::kNoteOn: {
            notes.insert(msg.NoteNumber());
            updateChordText();
            break;
        }
        case IMidiMsg::kNoteOff: {
            auto pos = notes.find(msg.NoteNumber());
            if (pos != notes.end()) {
                notes.erase(pos);
                updateChordText();
            }
...
void IChordTextControl::updateChordText()
{
    ChordAnalyser ca = ChordAnalyser(notes);
    std::string res = ca.getCommonName();
    if (!res.length()) SetStr(res.c_str());
    else {
